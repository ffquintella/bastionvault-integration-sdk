...
**Source of truth for behaviour:** [`specifications/`](specifications/README.md) · **Version:** 1.44.0 · 2026-09-24
...
rather than an extension of their mandate. **`v0.24.0` was cut on 2026-09-24 on the project
owner's direct instruction**, which is the R3 confirmation `v0.23.0` had as D-M12-24. **What the
instruction covered, and what it did not:** it authorised the tag. It did **not** rule on the
three specification amendments above, and it did **not** discharge D-M5-26's
`resilience.failover.read-once` release-checklist confirmation — §10 question 10 named that as a
rider and the rider was not separately answered, so both **stay open** rather than being read
into a one-line instruction. As before, the tag publishes nothing: no workflow here pushes to
NuGet, crates.io or PyPI.
...
10. **Cut `0.24.0`, and does the D-M5-26 carry-forward clear? — new at M12, 2026-09-24.** M12's
    work was on `main` under `## [Unreleased]`. **ANSWERED 2026-09-24: (a). `v0.24.0` is cut,
    committed and pushed on your instruction**, .NET-only under the `0.5.0` precedent, with no
    conformance level declared and nothing published. **The riders below were *not* answered by
    that instruction and remain open** — in particular D-M5-26's confirmation, which (a) said
    should be "recorded alongside it" and was not. Original framing: **Two things ride on the
    same answer.** First, D-M5-26 has carried a
