...
| **M10** 🔶 | Remaining engine bindings and identity → **declare Complete** | `IDN`, `RSC`, `FIL`, `LDP`, `RUS` | 9 booked, **5 landed** (`IDN-001`, `IDN-002` at slice a; `RSC-001`, `RSC-002`, `FIL-001` at slice b) | Large | R2 | **2 of 5** (slices a–b) | **Conformance level `Complete` declared in .NET (CNF-003 satisfied)** — subject to R-14 being answered. **Inherited from M8:** **`Auth.Userpass.ListUsersInfo`** ships under section 14's name while Appendix A nests it as `Auth.Userpass.Admin.*`; M10 builds the rest of that surface and must decide whether the member moves — cheap now, breaking once published (**R-29**, D-M8-46). **Framing record accepted 2026-09-22** ([DR-0017](decisions/0017-m10-remaining-bindings-and-identity.md)): five slices (a Identity+AssetGroups, b Resources+Files, c LDAP+CertLifecycle+Notifications, d Userpass.Admin.\*, e Rustion), dispatched serially — **slice b ran concurrently with slice c in an isolated worktree at the project owner's direction**, deviating from D-M10-1's serial default; the two slices' independent edits to `PublicApiSurface.txt`/`BastionVaultClient.cs` are reconciled once both land. New risks **R-35** (`identity.self` fixture has no real capture to author from) and **R-36** (`Files.Sync`'s credential fields ship as an opaque bag, no documented wire names) |
