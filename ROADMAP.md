...
baselined** and the gap is **R-30**, owned by M10 (D-M9-11). `TRN-031` came off the baseline
...
**Four risk rows open (R-30…R-33) and one sub-question closes.** R-27's standing instruction
...
- **Twice, secret material reached a container that does not redact.** `Pki.ExportCertificate`
  ...
  in a query string, which `ErrorPaths.Redact` does not cover (**R-32**), so it reached the
...
`PKI-030` (**R-30**), and sections 16–17 are M11's (**R-14**).
...
hold** — the server's queue-cap message (**R-30**), the PKI queue response shapes and the
`exportable` read-route question (**R-31**), and `Pki.GenerateIntermediate`'s `issuer_name`.
Those belong in M10's planning input, not its dispatch: a capture or the server source is a
dependency outside the SDK's control. **R-32** gates any return of the two export routes'
GET forms.
