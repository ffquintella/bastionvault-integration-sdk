...
| R-23 | **`tools/error-catalogue` compiles Appendix B §2's `+ a/b/c` form as a conjunction where the appendix means alternation, so five recognition rules can never fire.** `/` is alternation everywhere else in the §2 table, but the generator emits the alternatives following a `+` into `RecognitionRule.ContainsAll` — "extra substrings the message must **also** contain". Row 283 settles the intent past argument: a message cannot be both `is below min_decryption_version` and `not found on key`. Affected: **`BV-INPUT-102`** (SYS-042, landed M7b), **`BV-INPUT-103`** (SYS-091, landed M7c), **`BV-AUTH-011`** (machine binding — landed in M2 and **shipped since `v0.5.0`**), `BV-TRANSIT-004` (M8) and `BV-SSH-005` (M10) | **R3** | **Open, unowned.** Worst consequence is not a missing code but a wrong *retryability*: `BV-INPUT-103` is non-retryable and the fallback `BV-SERVER-005` is retryable, so a restore rejected for a tampered or corrupt file currently looks retryable — the same shape as D-M1c-25's `503` defect, where a guess suppressed a permitted retry and this one permits a forbidden one. Three consequences. (1) M7c works around it with an operation-local remap in `RestoreAsync` (D-M7-36) which is **designed to delete** when the generator is fixed. (2) Slice b's D-M7-18 test for `BV-INPUT-102` **passes by coincidence** — its message happens to contain both alternatives — so it proves less than it appears to; a rule with a coincidentally-passing test is worse than one with none. (3) M8 and M10 inherit a latent defect each, pre-booked before either milestone opens. (4) **The conformance fixture corpus masks it too** — `specifications/fixtures/errors/errors.recognition.bv-input-102.1.json` answers `namespace refuse cross-namespace`, containing *both* alternatives, so it passes identically under either semantics. The defect is therefore invisible to the unit test **and** to the cross-language corpus, which means Rust and Python would inherit the same false confidence at Stage 2 rather than catching it. **Fixing the generator without adding a single-alternative fixture leaves the corpus certifying the bug** — this is R-19's shape again, a fixture that passes for a reason unrelated to what it claims to prove. The fix is in the generator's parse plus a regeneration, not in `specifications/`, but it changes generated recognition behaviour for a shipped code, so CRS-004 makes it R3. It needs its own slot before M8. Evidence: [DR-0012](decisions/0012-m7-system-api-remainder.md) D-M7-36 |
