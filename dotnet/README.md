...
writing carries **104** IDs of **431** total (`tools/traceability/traceability.py --check`).
Counts per requirement-ID prefix:

| Prefix | Area | Gaps |
|---|---|---:|
| CNF | Conformance and quality (01) | 18 |
| DOC | Documentation requirements (16) | 16 |
...
**Why the `DOC` row still reads 16 rather than 1.** This baseline is a **test-reference**
...
match. The row fell from **19** to **16** at `0.24.0`, when `DOC-002`, `DOC-007` and
`DOC-024` gained an in-build test (`ErrorDocsDriftTests`). The remaining 16 do not close by
adding markers: four are enforced by Python checkers run from CI, five are asserted by
executed tests the scanner structurally cannot see — `BastionVault.IntegrationSdk.DocsSamples`
is a real test project that sits outside any `.Tests` directory — six have no assertion at
all, and `DOC-030` is held under D-M11-8. Tracked as **R-42**. The **48** `ITG` and **7** `TST` rows are milestone M12's live-server integration
