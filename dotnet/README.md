...
release it carries **131** IDs, of 425 total. Counts per specification section (grouped by
...
| 10 — SSH engine | SSH | 3 |
| 03 — Transport and protocol | TRN | 5 |
...
total is down from 234 at M4 to 131 here.

`AUT` (authentication), `BAT`, `CFG`, `DSC`, `EFF`, `KV`, `KV1`, `KV2`, `PAG`, `RES`, `SYS`,
`TOT` and `TRS` now carry **zero** gaps. `09` (PKI), `10` (SSH) and `12` (other engines and
identity) are not started; their counts above are every requirement ID those sections define.
