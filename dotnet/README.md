...
|---|---|---|---|
| **CCH-006** | `CacheWatcher`, the long-poll helper that raises per-topic change events | `CCH-006` is a **MAY**, so declining satisfies it. A looping long-poll helper is a lifecycle surface — disposal, cancellation, backoff, stop-on-`BV-AUTHZ-001` — and was declined rather than bolted on at the end of M8 (`decisions/0013-m8-transit-totp-and-efficiency.md`, D-M8-44) | M10 |
...
release it carries **120** IDs, of 425 total. Counts per specification section (grouped by
the requirement-ID prefix used throughout the specification and this baseline):

| Spec section | Prefix | Gaps |
|---|---|---:|
| 14 — Batch and request efficiency (cache coherence) | CCH | 1 |
...
total is down from 234 at M4 to 120 here.

`AUT` (authentication), `BAT`, `CFG`, `DSC`, `EFF`, `KV`, `KV1`, `KV2`, `PAG`, `RES`, `SSB`,
`SSH`, `SYS`, `TOT` and `TRS` now carry **zero** gaps — **section 10 clears entirely at M9**.
...
(`decisions/0016-m9-pki-and-ssh.md`, D-M9-11; `ROADMAP.md` R-30). `12` (other engines and
...
- The **PKI engine** (09): roles, issuance (`Issue`, `Sign`, `SignVerbatim`), certificates
  ...
  and both the outbound CSR and inbound sign-request queues. **The SDK signs no certificate
  and generates no key** — every route posts to the server and returns what it minted.
