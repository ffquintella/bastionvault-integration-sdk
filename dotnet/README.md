...
release it carries **114** IDs, of 425 total. Counts per specification section (grouped by
...
total is down from 234 at M4 to 114 here.

`AUT` (authentication), `BAT`, `CFG`, `DSC`, `EFF`, `FIL`, `IDN`, `KV`, `KV1`, `KV2`, `PAG`,
`RES`, `RSC`, `SSB`, `SSH`, `SYS`, `TOT` and `TRS` now carry **zero** gaps, and so does `CCH` —
**section 10 clears entirely at M9**. `IDN` is M10 slice a's own close: `IDN-001` (client-side
base64url encoding) and `IDN-002` (the `ForMe` group-share filter, a documentation MUST) both
ship with `Identity.Groups`, `Identity.Sharing` and `Identity.Owner`
...
asymmetry) ship with `Client.Resources`. **Section 09 does not** clear: its single remaining
gap is `PKI-030`, whose queue-cap limb requires recognising a server message that no document
states, so it is held back rather than guessed (`decisions/0016-m9-pki-and-ssh.md`, D-M9-11;
`ROADMAP.md` R-31). `12` (other engines and identity) has two slices closed (identity kernel
and asset groups at slice a; resources and files at slice b); its remaining counts above are
`LDP` and `RUS`, still every requirement ID those areas define.
