...
| Option | Why rejected |
|--------|--------------|
| Treat `RUS-002`'s "node-local, no failover" as requiring a new client-side capability | `DSC-045`'s internal node-local seam already exists (M5): `ExecuteShapedAsync`'s `nodeLocal` parameter, in production use since M5 by `Sys.Seal`/`Sys.Unseal` (`SysOperations.cs:274,291`). RUS-002 is a *citation* of an existing mechanism, not a new one. **Correction (slice e handback, 2026-09-22):** this row originally said the seam was "reused by `sshbroker` at M9" — checked at slice e's R2 gate and found false: `SshBrokerOperations.cs` sets `nodeLocal: true` nowhere. The mechanism's only pre-M10 callers are `Sys.Seal`/`Sys.Unseal`; slice e is the seam's second real caller, not its third |
